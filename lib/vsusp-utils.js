...
function convertSuspensionData(suspensionsIniData, tyresIniData) {
  const { type, trackWidth, wheelOffset, upperMountX, upperMountY, upperBallJointX, upperBallJointY, lowerMountX, lowerMountY, lowerBallJointX, lowerBallJointY, strutUpperMountX, strutUpperMountY, strutLowerMountX, strutLowerMountY } = suspensionsIniData;
  ...
  const frameCenterToUpperMountX = (trackWidth / 2) - (upperMountX || strutUpperMountX);
  const frameBottomToUpperMountY = (upperMountY || strutUpperMountY) - lowerMountY;
  ...
  // steering knuckles (strut)
  // ----------------
  // hub to strut axis --> strut lower mount X + wheel offset
  // strut inclination angle --> Math.asin((upper strut mount Y - lower strut mount Y) / strut length) * 180 / Math.PI
  const hubToStrutAxis = strutLowerMountX + wheelOffset;
  const strutInclinationAngle = Math.asin((strutUpperMountY - strutLowerMountY) / Math.sqrt(Math.pow(strutUpperMountX - strutLowerMountX, 2) + Math.pow(strutUpperMountY - strutLowerMountY, 2))) * 180 / Math.PI;
...
function generateSuspension(data) {
  ...
  const suspension = [
    ...
    `knuckles.hub_to_strut_axis:${normalize(hubToStrutAxis)}`,
    `knuckles.strut_incl:${strutInclinationAngle * 100}`,
